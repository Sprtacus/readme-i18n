"""readme_i18n/hook.py – Git-Hook-Entry-Point"""
...
def detect_repo_root() -> Path:  # unchanged
    try:
...
# ---------------------------------------------------------------------------
# Translation logic (geändert)
# ---------------------------------------------------------------------------

def load_api_key() -> str | None:  # unchanged
    load_dotenv()
    return os.getenv("DEEPL_API_KEY")


def translate_body(txt: str, tr: deepl.Translator, lang: str) -> str | None:
    """Übersetzt den *lesbaren* Teil des README, schließt Code/Emoji explizit aus."""
    # 1) Segmente schützen
    cleaned, mapping = protect_segments(txt)

    # 2) DeepL‑Aufruf (kann bei Quell‑Sprache ‚EN‘ entfallen)
    try:
    ...
    # 3) Platzhalter zurückwandeln
    return restore_segments(translated, mapping)


def build_translations(readme: Path, key: str, cfg: Config) -> List[Path]:  # unchanged except translate_body
...
def main(argv: List[str] | None = None) -> int:  # unchanged
